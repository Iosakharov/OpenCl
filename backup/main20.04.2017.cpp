...
for (int i=0; i<MEM_SIZE; i++)
{
		...
		srcB[i] = 1.0/(i+1);
		std::cout << "  |  " << srcB[i];
		in[i] = ASort[i] = ASort2[i] = (1+rand()%100);
		std::cout << "  |  " << in[i] << "* \n";
}
...
{/*
	if (!start)
	{
	ret = clEnqueueReadBuffer (command_queue, ABuffer[0], CL_TRUE, 0, MEM_SIZE * sizeof(cl_uint), ASort, 0, NULL, NULL);
	std::cout << ret << '\n';
}*/
	*len_cl = (uint)(int)length<<1 ;
	*inc_cl = (uint)(int)inc;
...
std::cout << "Write" << '\n';
	ret = clEnqueueWriteBuffer(command_queue, length_mem, CL_TRUE, 0, sizeof(cl_uint), len_cl, 0, NULL, NULL);
	std::cout << ret << '\n';
	ret = clEnqueueWriteBuffer(command_queue, inc_mem, CL_TRUE, 0, sizeof(cl_uint), inc_cl, 0, NULL, NULL);
std::cout << ret << '\n';
*/
...
std::cout << ret << '\n';
	ret = clSetKernelArg(kernel8, 2, sizeof(cl_uint), (void*)&len_cl);
std::cout << ret << '\n';
	ret = clSetKernelArg(kernel8, 3, sizeof(cl_uint), (void*)&inc_cl);
	std::cout << ret << '\n';
	ret = clSetKernelArg(kernel8, 4, sizeof(cl_mem), (void*)&memobj);
	std::cout << ret << '\n';
  current = 1-current;
